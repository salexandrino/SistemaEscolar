...
public class TurmaDisciplinaProfessorRepository extends BaseDAO {
    ...
    public List<br.com.synge.academico.dtos.GradeProfessorItemDTO> consultarGradeProfessor(UUID tenantId, UUID idProfessor) {
        String sql = """
                ...
                """;
        List<br.com.synge.academico.dtos.GradeProfessorItemDTO> itens = new ArrayList<>();
        try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setObject(1, tenantId);
            ps.setObject(2, idProfessor);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    br.com.synge.academico.dtos.GradeProfessorItemDTO item = new br.com.synge.academico.dtos.GradeProfessorItemDTO();
